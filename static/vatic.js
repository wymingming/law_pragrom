...
function createFramesManager(){
  return new FramesManager();
}
 ...
 */
function extractFramesFromZip(config, file, fileName) {
  return new Promise((resolve, _) => {
      file.then((zip) => {
        let totalFrames = 0;
        for (let i = 0; ; i++) {
          let file = zip.file(fileName + "\\" + i + config.imageExtension);
          if (file == null) {
            totalFrames = i;
            break;
          }
        ...
        resolve({
          ...
          getFrame: (frameNumber) => {
            return new Promise((resolve, _) => {
              let file = zip.file(fileName + "\\" + frameNumber + config.imageExtension);
              file
                .async('arraybuffer')
                .then((content) => {
...
function createAnnotatedObjectsTracker(framesManager){
  return new AnnotatedObjectsTracker(framesManager);
}

// export{
//   createFramesManager,
//   createAnnotatedObjectsTracker,
