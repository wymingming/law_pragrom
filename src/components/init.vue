      ...
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="未标注视频" name="Untagged">
          <div class="box">
            <div>
              <!-- 这里写列表 -->
              <el-table :data="Untaggedurls" style="width: 100%">
                <el-table-column prop="name" label="视频列表" width="180px">
        ...
        <el-tab-pane label="已标注视频" name="Tagged"></el-tab-pane>

        <el-tab-pane label="所有视频" name="All">
          <div class="box">
            <div>
              <!-- 这里写列表 -->
              <el-table :data="Allurls" style="width: 100%">
                <el-table-column prop="name" label="视频列表" width="180px">
                  <template slot-scope="scope">
                    <el-button
                      size="mini"
...
export default {
  ...
  mounted() {
    ...
    this.$http
      .get(
        "http://192.168.1.120:9090/annotation/video/find?currentPage=1&pageSize=6"
      )
      .then(
        response => {
          ...
          this.Alltotal = parseFloat(response.body.result.total);
          //console.log(url);
        },
        response => {
          alert("获得视频列表失败");
  ...
  methods: {
    ...
    AllhandleCurrentChange(val) {
      ...
      this.$http
        .get(
          `http://192.168.1.120:9090/annotation/video/find?currentPage=${val}&pageSize=6`
        )
        .then(
          response => {
            url = response.body.result.records;
    ...
    handleEdit(index, row) {
      ...
      if (
        ...
        confirm("忘记提交了，之前提交的区间会被清空，您确定吗？")
      ) {
        this.starttime = [];
        this.endtime = [];
        this.video = row;
        this.playerOptions.sources = [
          { type: "video/mp4", src: "http://" + row.address }
        ];
      }
    },
    handleClose(index) {
    ...
    chooseVideo(video) {
      ...
      if (
        ...
        this.video = video;
        this.playerOptions.sources = [
          { type: "video/mp4", src: "http://" + video.address }
        ];
      }
    },
    handleClick(tab, event) {
      let url;
      if (tab.name == "Untagged") {
        this.$http
          .get(
            "http://192.168.1.120:9090/annotation/video/find?currentPage=1&pageSize=6"
          )
          .then(
            response => {
              ...
              this.UnTaggedtotal = parseFloat(response.body.result.total);
              //console.log(url);
            },
            response => {
              alert("获得视频列表失败");
